...
The word Bloom comes from the name of the scientist **Burton Howard Bloom** who invented this.
:::
...
and we only need to check if a specific condition is true or false.
...
such that the same short isn't shown again.
...
Yet when for a key, if the index isn't set to '1', then the output is **definitely false**.
:::

<ThemeImage src='/img/bloom-filter.excalidraw.png' alt='bloom-filter' />
...
There exists a standard formula that describes the parameters that must be considered.

Formula to calculate filter size can be found [on internet](https://hur.st/bloomfilter/).

:::warning changing size of Bloom filter isn't easy
Changing size of Bloom filter isn't straight forward.
We can't just create another bigger bit array and copy the data there
because if we change the array size then the 'mod' value also changes.
the existing calculations become invalid.
:::
...
Refer to the [bit masking](../algorithms/bit-masking.mdx) and
[bit vectors](../computers/bit-manipulation.mdx)
