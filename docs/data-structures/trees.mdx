...
All nodes connected and having just n-1 edges indirectly means that there
are no multiple connections between nodes.
:::
...
we recursively work on subtrees until we reach the leaf nodes.
:::
...
$$\text{Height of a node}=\text{Max height of its children} + 1$$.
...
Whenever a node has specific subtree empty, we consider the height of that empty subtree as -1.
...
Then we always add **+1** for the edge connecting the node to its child.
