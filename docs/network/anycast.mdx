...
It's a load balancing setup in which an application is hosted behind different routing domains but with same IP address.
...
Cloudflare hosts 1.1.1.1 deploys this IP address on multiple data centers.
Still, the DNS queries goes to the nearest one based on the nearest path selected by the router.
:::
