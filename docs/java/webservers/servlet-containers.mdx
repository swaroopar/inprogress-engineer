...
Servlet containers handle the HTTP requests and responses.
They also know which Java bean to call for each request.

A container receives the request.
It runs through the HTTP filters.
Then it calls the Java class or method that handles the request.

<ThemeImage src='/img/java-webserver-servlet.excalidraw.png' alt='servlet-container' />

:::tip Servlet containers vs HTTP Servers
Plain HTTP servers only implement the HTTP protocol. They serve only static content.

Servlet containers can serve dynamic content.
:::

## Convert HTTP to Java Objects

In tomcat, the web server turns the raw HTTP request data into a structured Java object.
...
public class NioServer {
    public static void main(String[] args) throws Exception {
        ...
        // Register ServerSocketChannel with Selector for OP_ACCEPT
         // adds the created channel to the created epoll_create() table
         // and tells about the events that are useful for it.
         // this means its ready to be accepted. Inform any channels that are ready to be accepted.
        serverChannel.register(selector, SelectionKey.OP_ACCEPT);

        while (true) {
            // Block until at least one channel is ready. Meaning its put to sleep.
            // calls the epoll_wait() method to wait until an event occurs.
            // This part of thread is put to sleep until there is an event for any of the registered channels.
            selector.select();
