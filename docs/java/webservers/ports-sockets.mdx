...
When kernel sees data for a specific thread, it then schedules the thread that was put to sleep for a specific file descriptor.
The application will continue executing which means, it will **accept** this and return a **socket** object.
