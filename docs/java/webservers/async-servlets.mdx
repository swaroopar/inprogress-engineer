...
<ThemeImage src='/img/async-http-servlets.excalidraw.png' alt='Asynchronous Servlets' />
...
3. Worker Thread - Write the response back to client.

It's important to understand that the request servlet thread sends the response object to asynchronous thread.
The callback method on this object is then called by asynchronous thread when the
processing finishes. This method then starts submits a task to worker thread pool
to pickup the send response to the topic. See [this](./thread-communication.mdx) for more details on thread communication.
...
There exists no difference of the HTTP clients. The socket in which the request came in will remain connected.
...
<ThemeImage src='/img/java-listener-thread.excalidraw.png' alt='thread-creation' />

## Spring Reactive

In case of normal async servlets, we just offload the task to a thread pool outside of the servlet container.

In spring reactive though, the async process works similar to the **event loop** in NodeJS.
