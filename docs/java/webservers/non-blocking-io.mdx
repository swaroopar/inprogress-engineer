...
In this case, the IO refers to the network IO which is used to exchange HTTP messages between clients and servers.

<ThemeImage src='/img/java-nio.excalidraw.png' alt='nio-message-processing' />
...
1. JVM listens on the server port. For example, 8080. This is the acceptor/main thread.
2. When a request arrives at this port,
...
There exists always one TCP session used to exchange multiple HTTP messages between clients.
the thread in the BIO case is blocked by one connection until it's fully closed.
:::
...
3. Poller thread is scheduled by kernel if there is any file descriptor waiting for it.
4. Poller then queues the request and wakes up a worker thread using [Futex.](../../computers/futex.mdx).

:::danger misconception of callbacks
All documents speak about **callbacks** used by worker threads to update the response.
There exists no callback as such.

The actual objects that the poller thread creates after registering a socket for polling is sent across thread pools.
The worker or async threads call the method on these objects which sets the buffer directly on the object
...
4. When available, kernel wakes up the thread.
