...
Everything in this document can be applied to all IO and not just regular hard disk files.

:::important Java file creation
...
In case of files, buffers are just a bridge between JVM and the underlying file system.
It's fixed amount of memory allocated in JVM heap and
the system call copies data from kernel space to this buffer and then JVM can read from it.

When we ask for the next set of bytes,
the system call will copy the next set of bytes to the same buffer.

:::warning Only type with actual buffer
This is the only type which actually has a buffer.
The rest of the types have the word buffer in it,
but they don't have actual buffer.
:::
...
:::danger avoid system calls
Since system calls are costly, using memory mapped files avoid multiple system calls.
...
Since loading any file from ROM into memory includes the [page cache](../computers/memory-paging.mdx#swapping),
...
Not that they're nothing to do with files and
are normally not used at all for file IO.
It's internally used by network IO to copy data directly
to and from JVM memory.
:::

## Stream vs Buffer

:::important IO Stream in Kernel
Streams in Java are nothing but the operating system's IO using [file descriptors](../storage/file-descriptors.mdx).
...
<ThemeImage src='/img/file-stream-buffer.excalidraw.png' alt='stream and buffer' />
