...
It's coming from a concept called **[Semantic Web](./semantic-web.mdx)**
where data is linked across different sources on the web.
:::

<ThemeImage src='/img/data-model-json-ld.excalidraw.png' alt='JSON-LD Example' />
...
It's a way to represent RDF data model in JSON format.

:::warning URI vs URL
It's key to understand that the IDs here are URIs and not URLs.
...
**Remember:** @type is a special predicate in RDF which is also used in JSON-LD.
:::
...
In JSON-LD, the URIs just refer to other resources from other sources.
...
- **"vocab**: When used, call fields are referred directly from this vocab.
