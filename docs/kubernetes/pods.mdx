...
Only thing that exists is the [pause container](pause.mdx) container
which is what the kubelet creates as the root of the pod.

## Preparation

Pod preparation is similar to [container preparation](../containers/isolation.mdx#container-creation-process).
