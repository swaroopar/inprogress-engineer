...
There exists no real relation to circle or triangle here.
...
But this input variable isn't really used by the function.
...
the libraries that build trigonometric functions use Taylor series formulae to calculate these values.

## GPS Co-ordinates

GPS co-ordinates are represented in terms of latitude and longitude which are angles.
These angles are then converted to Cartesian co-ordinates using sine and cosine functions
to determine the position on the Earth's surface.

Without the Cartesian co-ordinates,
it's not possible to calculate the exact distance between two different GPS co-ordinates.

:::important GPS is just angles

- **Latitude** - It's the angle from the equator to the point on the Earth's surface.
  It's represents the north-south position of the point on the Earth's surface.
- **Longitude** - It's the angle from the prime meridian to the point on the Earth's surface.
  It represents the east-west position of the point on the Earth's surface.
  :::
