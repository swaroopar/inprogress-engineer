...
The only difference is that JavaScript is single-threaded and Java is multi-threaded.
This means, same thread is used to execute the code and also to execute the asynchronous tasks.
...
Directly execute something else whenever the first async step completes.

:::important How handover happens?
When the async process completes,
it calls the functions on the Promise object which asks the JavaScript engine
to restart the paused execution of the main code by placing the result in the place
where the async process was called.

When the compiler sees an async function,
it replaces that code with a state machine where the existing execution is paused
and the state of the execution is stored in the heap.
When the async process completes,
the state machine uses this heap object to resume the execution from where it exactly stopped.
:::
