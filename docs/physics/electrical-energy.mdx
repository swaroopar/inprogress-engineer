...
<ThemeImage src='/img/atom-internal.excalidraw.png' alt='current' />

## Electromagnetic Field

**Electric field** - It's always there between electrons and protons and also between atoms.
In the normal case the field is neutral, since the same force is applied on both positive and negative charges.
...
<ThemeImage src='/img/em-field-generation.excalidraw.png' alt='current' />
...
There exists no magnetic field.
:::
...
The electric fields, though, are always between the live wire with the actual voltage and neutral which is at 0V.
...
the wires are ensured to have high resistance so that the current is low.
:::

While transferring high voltage from a power grid,
...
- If resistance is high, the electrons struggle more to move - less current flows.
- If resistance is low, it's easier for electrons to move - more current flows for the same electric field.
