...
Additionally, the same client code that we send as part of CSR is also sent here.  
:::
...
Still, it only renders the static HTML. All dynamic content comes from the hydration process.
:::
