...
1. Value of Current Privilege Level(CPL) Register. This is where the ring value is set.
   ...
   CPU knows in which security level it's executing.
...
[Page table](./memory-paging.mdx#page-table-structure) allocated to a process has permissions on each page.
...
When kernel starts systemd, it starts with ring 3.
