...
such that the CPU can continue where it actually left off.
:::
...
There exists one kernel interrupt stack per CPU.
...
There exists one interrupt stack per CPU and this is used only for hardware interrupt handling.
