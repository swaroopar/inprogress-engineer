...
<ThemeImage src='/img/ram-physical-structure.excalidraw.png' alt='ram-structure' />
...
There exists just a serial presence detect (SPD) chip that contains information about the RAM stick,
...
The 4 ranks and out of which 2 can be accessed at a time.
:::
...
<ThemeImage src='/img/ram-memory-translation.excalidraw.png' alt='ram-structure' />
...
<ThemeImage src='/img/ram-memory-cell.excalidraw.png' alt='ram-cell' />
...
<ThemeImage src='/img/ram-lines.excalidraw.png' alt='ram-bit-line' />

:::tip memory matrix
The picture above shows only a small size of the matrix.
In reality, the matrix can be huge with many rows and columns.
:::
...
Still, the CPU cache lines are 64 bytes.
The RAM must transfer 8 times the data to fill the cache line.
...
There exist two types of refresh happening in DRAM:
