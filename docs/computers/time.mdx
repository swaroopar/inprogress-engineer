...
<ThemeImage src='/img/time-source-calculation.excalidraw.png' alt='time-source' />
...
and then starts its own counter again the last fetched value.
This is exactly why the server time continues to run even when there is no network connection.
:::

:::danger Time on Unix systems
On Unix, whenever we ask for the system time, it always returns whatever the Unix seconds counter shows.
...
ISO 8601 is now such standard which represents the UTC is a human-readable and machine friendly format.
