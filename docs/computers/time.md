...
Here times calculation standards means, the set of rules to calculate the current time.
...
2. UTC - Co-ordinated universal time.
   This is the new standard and only used at present.
   In this standard, they define a second based on how many times an atom needs to vibrate to consider it a second.
   This is the most accurate time standards.

:::important UTC is just like a stop watch
The clocks implementing UTC, just keep counting seconds from its start time.
It's the responsibility of the clients to convert this number of seconds lapsed value into calendar data and time.
...
The implementation of it's what generates the time and is used by systems such as NTP, GPS etc., as an original source.
...
and then starts it's own counter again from there.
:::
...
Every time source always says only the number of seconds lapsed from this epoch point.

:::info meaning of epoch
Meaning of the word **epoch** itself means a fixed point in time from which a new era or period is measured.
:::
