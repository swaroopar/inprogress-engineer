...
<ThemeImage src='/img/computers-cpu-cache-lines.excalidraw.png' alt='cpu-cache' />

## Cache Lines

Cache line is nothing but the cache entry size.
...
L3 has a directory component which knows which address is currently in the cache of which core.
This is also what's used for serializing lock requests.

:::danger Lock requests for memory
If a core wants to lock an address,
it will send out the 'LOCK' control command to the L3 controller,
which then sets the ownership of the memory address to the specific core
and then sends another control message to all other cores which has the same address in its cache.

If two threads ask for the lock on same address,
then this is where the requests get serialized and
only the first request gets the lock acquired.
:::
...
<ThemeImage src='/img/computers-cpu-shared-lines.excalidraw.png' alt='cpu-cache-lines-shared' />

:::info cache line serialization
In the diagram we see that the L3 cache line is shared across multiple cores.
Still, when a core accesses a specific cache line,
...
<ThemeImage src='/img/computers-cpu-cache-mmu.excalidraw.png' alt='Cache Addressing' />

### Cache Coherence

Cache coherence is the mechanism where when a specific memory address that's currently cached
is updated by one of the cores,
then the cache lines of all other cores are also marked as dirty.
See [L3 Directory](#l3-directory) section for more details.

:::important cache coherence and CAS
All [CAS](../java/atomic.mdx#cas---compare-and-save) features are implemented using this hardware feature of cache coherence.
:::
...
<ThemeImage src='/img/cpu-cache-split.excalidraw.png' alt='Cache Key Generation' />
...
This is much like how we calculate the index key for hash tables.
