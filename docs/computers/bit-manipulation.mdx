...
It uses [boolean algebra](../mathematics/boolean-alegbra.mdx) operations for the same.

:::important bit vectors are important
...
It's useful to understand how data manipulation and extraction in bit vectors work.
:::
...
Such a solution can be useful to check if the bit vector has **only one bit** set to 1 or not.
$$\text{x } \& \text{ (x-1)}$$ will be zero only when there is only one bit set to 1 in the bit vector.
:::

:::tip what $$\text{x } \& \text{ -x}$$ means?
Important nuance to keep in mind is,
between x and -x the bits are same until the first/lowest 1 (starting from the right bit),
after that all bits are opposite between x and -x.

See [arithmetic negation](../mathematics/boolean-alegbra.mdx#arithmetic-negation) for more details.
:::
...
Or other way to look at it, is to say move the bit at that **index** position to $$0^{th}$$ position.
:::
...
- Checking returns boolean - true or false.
  This is done by performing an AND operation and checking whether the result is zero or not.
