...
Normally for regular file operations,
There exists just one file descriptor created for a file which is used for both reading and writing.

But in case of sockets, the kernel creates two file descriptors.
One for reading and one for writing.
This means, the producer gets one file descriptor
and the consumer gets the another file descriptor to which it can write responses to.

:::warning why do sockets have two file descriptors?
Sockets are simply backed by buffers in memory.
When a single buffer is used for both reading and writing,
then same process will write and also read from the same buffer which is useless.
:::
