...
There exists two ways to mainly do it.
...
There exist three ways to do DFS. These are important for graphs of type binary trees.
...
**Knowing this small nuance then is key to understand the traversal algorithms.**
:::
...
This is nothing but BFS but the term applies only to trees and not for graphs.
We transverse the tree at each level and
then also apply different logic at each level if necessary.
