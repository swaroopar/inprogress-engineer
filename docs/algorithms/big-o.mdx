...
it's been used in practice to represent rate of growth.
...
In case of ArrayList appending an element scenario which is always mentioned in all books will be then,
...
You can also say that the door can be maximum 10 feet tall
...
When the input reaches huge values,
...
When we say O(2N) is same as O(N), since the shape of the graph is same.
The constant 2 doesn't change the shape of the graph, it just scales it.

Similarly, when we say O(N + log N) it's same as O(N).
Here, we're saying that as input size increases,
...
How to look at this is, if the entire runtime is log N
...
**It's key to keep in mind that different algorithms behave better at different input sizes.
...
Big O is about the limiting behavior as $n \rightarrow \infinity$,
not about which algorithm is faster for your actual use case.
...
- Plus some steps then runs on all halves at each step.
  :::
