...
Naive pattern matching runs at $$0(M \times N)$$.
...
Still, length starts it's count from 1 where as the LPS array index from 0.
...
When there is a mismatch, we move only the pointer of the pattern string back.
