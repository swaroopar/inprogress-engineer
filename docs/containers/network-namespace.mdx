...
What exactly happens inside the kernel during this can be understood
[in detail in the network devices doc](../linux/network-devices.mdx#how-physical-and-virtual-devices-created).
...
See [Linux network device](../linux/network-devices.mdx#kernel-as-router) to understand how this is achieved.

:::warning no restriction on connection between namespaces
Namespaces only provides isolation of memory structures.
There exists no restriction on how the devices in different namespaces can be connected.
...
See [this page](../linux/network-devices.mdx#how-multiple-ip-addresses-on-the-same-device-work) for more information.
...
no request ever reaches the host itself.
See [routing](../network/routing.mdx#router-and-switch-integration) document for more information.
