...
When a container process is started, the process is attached to its unique namespace.
...
It basically means, the process in this namespace sees its own /lib, /bin, /sbin, /usr, /home, folders.

:::warning only own rootfs
Containers have only their own rootfs but the kernel is used from the host.
All the system calls will go via the host kernel.
:::
...
:::danger mount namespaces always inherit
Mount namespace is the only namespace that inherits the entire mount details from the parent namespace.
:::
...
**Remember:** The bind mounted volume stays visible even though it was mounted from the backup location.
