...
or any other condition. This means, each miner can be mining any transaction that it can pick from the mempool.
...
5. This blockchain now becomes the longest one becomes the active one and other blockchain is discarded.
   :::
...
There exists a limit on number of nodes a node in the network connects to.
:::
...
in case of an bank account, the final balance of an user isn't stored anywhere.
