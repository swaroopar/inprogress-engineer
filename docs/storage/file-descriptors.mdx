...
It has pointers to the corresponding [**inode**](./inodes.mdx) structure and
also pointers to different operations that can be executed on it.

A same file can have multiple instances of file structure if it's opened multiple times.
This is useful when multiple processes want to access the same file from different offsets.
:::
