...
This page describes only the inode data structure for real files.
:::

:::tip inode is an ext4 concept
inode is just a Linux ext4 file system concept.
...
6. Actual size of the file - this is needed to know where to stop in the last disk page used by the file.

:::important inode doesn't hold the most important information
It's key to keep in mind that the inode neither contains the file name nor
the file data. It's in the dentries.
:::

There exists an inodes cache maintained by kernel on memory.
...
There exists a dentries cache maintained by kernel on memory.
It keeps adding data to it whenever files are accessed.
