...
This is key.
Later we can just override any implemented method as necessary.
:::
...
2. If the embedded struct is a pointer,
   then the pointer is initialized to nil. Then the calling any method on that struct will fail.
