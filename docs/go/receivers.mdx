...
Based on the struct type, either it's value or pointer, the corresponding method is available.
...
Otherwise just use the value receiver.
...
It's key to understand that, the receiver defined in the method is
nothing but the first argument of the method after compiling.

the receiver data can be accessed directly in the method.
